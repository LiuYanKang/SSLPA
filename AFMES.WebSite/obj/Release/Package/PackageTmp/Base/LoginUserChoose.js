...
function loadMubeaUserList(pagerParams) {
    datagrid.loading();
    $.srv("Base", "QueryMubeaUser", {
    ...
    if (e.select) {
        ...
        for (var i = 0; i < e.records.length; i++) {
            ...
            for (var li = 0; li < selectedItems.length; li++) {
                if (selectedItems[li].User_Name == e.records[i].User_Name) {
                    hasValue = true;
                    break;
                }
            }
            if (!hasValue)
                selectedItems.push({
                    User_Name: e.records[i].User_Name,
                    Email: e.records[i].Email
                });
        }
    } else {
        for (var i = 0; i < e.records.length; i++) {
...
function onGridUpdate(e) {
    ...
    for (var i in source) {
        selected.each(function () {
            if (this.User_Name == source[i].User_Name) {
